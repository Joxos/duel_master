...
# Summon Type # 召唤类型
SUMMON_TYPE_NORMAL = (
    0x10000000  # 通常召唤(EFFECT_SUMMON_PROC,EFFECT_SET_PROC 可用Value修改數值)
)
...
RESET_MSCHANGE = (
    0x08000000  # 从怪兽区到魔法区，或者从魔法区到怪兽区(move_to_field()、寶玉獸)
)
# 组合时点
...
RESETS_WITHOUT_TEMP_REMOVE = (
    0x56E0000  # RESETS_STANDARD-RESET_TEMP_REMOVE-RESET_LEAVE+RESET_OVERLAY
)
...
EFFECT_FLAG_ABSOLUTE_TARGET = (
    0x0040  # Target Range固定為某個玩家的視角所見的自己/對方(internal)
)
...
EFFECT_FLAG_CAN_FORBIDDEN = (
    0x0200  # 可被禁止令停止適用的效果（與EFFECT_FLAG_CANNOT_DISABLE並用）
)
...
EFFECT_FLAG_CONTINUOUS_TARGET = (
    0x8000000  # 建立持續對象的永續魔法/永續陷阱/早埋系以外的裝備魔法卡
)
...
EVENT_BE_MATERIAL = (
    1108  # 作为同调/超量/连结素材、用于升级召唤的解放、作为仪式/融合召唤的素材
)
...
EVENT_SPSUMMON_SUCCESS_G_P = (
    1117  # EFFECT_SPSUMMON_PROC_G特殊召唤成功前(只能使用EFFECT_TYPE_CONTINUOUS)
)
...
EVENT_REMOVE_COUNTER = (
    0x20000  # 去除指示物时(A指示物)，Card.RemoveCounter()必須手動觸發此事件
)
...
DUEL_RETURN_DECK_TOP = (
    0x80  # 回卡组洗切的卡放到卡组最上方（不洗牌模式下曾经的默认行为）
)
