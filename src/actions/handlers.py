...
from actions.events import DrawCard, NormalSummon, SetPhase, Skip
...
def normal_summon_available(event: GetAvailableActions):
    ...
    player_correct = duel.phase.player == player
...
def normal_summon(event: NormalSummon):
    ...
    card.status = CardStatus(EXPRESSION_WAY.ATTACK, FACE.UP)
...
def change_phase(event: GetAvailableActions):
    duel, player = event.duel, event.player
    if duel.phase.player == player:
        actions = []
        consequence = CONSEQUENCE if duel.turn_count != 1 else CONSEQUENCE_OF_TURN_1
        for phase in consequence[duel.phase.phase]:
            player = player if phase != PHASE.DRAW else duel.opponent_player(player)
            actions.append(
